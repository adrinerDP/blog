<template>
  <Layout>
    <content-header :image="$page.blog.image" :staticImage="false" :opacity="0" />
    ...
    <div class="container sm:pxi-0 mx-auto overflow-x-hidden text-gray-800 dark:text-gray-500">
      <div class="lg:mx-32 md:mx-16 sm:mx-8 mx-4 pt-8">
        <section class="post-header container mx-auto px-0 mb-16 text-center">
          <h1 class="text-gray-800 dark:text-gray-400 font-extrabold tracking-wider mb-6">
            {{ $page.blog.title}}
          </h1>
          <span class="tracking-wide text-sm">
            ...
            &nbsp;&middot;&nbsp;
            읽는 데 <span class="font-semibold">{{ $page.blog.timeToRead }}분</span>
          </span>
        </section>
      </div>
      ...
      <div class="lg:mx-32 md:mx-16 px-4">
        ...
        <section class="post-tags container mx-auto relative py-10">
          <g-link v-for="tag in $page.blog.tags" :key="tag.id" :to="tag.path" class="text-xs bg-transparent hover:text-blue-700 py-2 px-4 mr-2 border hover:border-blue-500 border-gray-600 text-gray-700 dark:text-gray-400 rounded-full">
            {{ tag.title }}
          </g-link>
        </section>
      </div>
              ...
              <div class="w-full flex justify-center md:w-10/12 mb-4 text-center">
                <div class="mb-2 sm:mb-0 w-full">
                  <div class="md:flex p-6 pl-0 self-center">
                    <g-image :src="$page.blog.author[0].image" class="h-16 w-16 md:h-24 md:w-24 mx-auto md:mx-0 md:mr-6 rounded-full bg-gray-200" />
                    ...
                    <div class="text-center md:text-left">
                      ...
                      <div v-if="authors.length>0" class="post-authors font-light text-sm pt-2">
                        Among with
                        <g-link class="font-normal" :to="author.path" v-for="author in authors" :key="author.name">
                          {{author.name}}
                        </g-link>
                      </div>
                      <div class="font-light tracking-wider leading-relaxed py-4">{{ $page.blog.author[0].bio }}</div>
                      <div>
                        <a :href="$page.blog.author[0].facebook" target="_blank" rel="noopener noreferrer" class="hover:text-blue-500">
                          <font-awesome :icon="['fab', 'facebook']" />
                        </a>
                        &nbsp;
                        <a :href="$page.blog.author[0].twitter" target="_blank" rel="noopener noreferrer" class="hover:text-blue-500">
                          <font-awesome :icon="['fab', 'twitter']" />
                        </a>
                        &nbsp;
                        <a :href="$page.blog.author[0].linkedin" target="_blank" rel="noopener noreferrer" class="hover:text-blue-500">
    ...
    <section class="post-related pt-10" v-if="relatedRecords.length>0">
      <div class="container mx-auto">
        <div class="text-center">
          <h4 class="font-light my-0">추천 게시글</h4>
        </div>
        <div class="flex flex-wrap justify-center pt-8 pb-8">
          <CardItem :record="relatedRecord.node" v-for="relatedRecord in relatedRecords" :key="relatedRecord.node.id" />
        </div>
      </div>
    </section>
  </Layout>
