<template>
  <div class="bg-gray-100 dark:bg-gray-900 text-black dark:text-gray-400 text-center text-sm">
    <div class="container mx-auto py-16">
      <div class="mb-4">
        <p class="mb-2 font-semibold">Maintained with <font-awesome icon="heart" /> by Junsu Lee</p>
        <p><font-awesome icon="copyright" /> {{ new Date().getFullYear() }}, {{ $static.metadata.siteName}}</p>
      </div>
      <div class="mb-4">
      ...
      <div class="mb-4">
        <ul class="list-reset flex justify-center">
          <li v-for="navItem in $static.metadata.footerNavigation" :key="navItem.name" class="px-4">
            <g-link :to="navItem.link" :title="navItem.name" v-if="!navItem.external" class="hover:text-blue-500">
              {{ navItem.name }}
            </g-link>
            <a :href="navItem.link" target="_blank" :title="navItem.name" v-if="navItem.external" class="hover:text-blue-500">
              {{ navItem.name }}
            </a>
          </li>
        </ul>
